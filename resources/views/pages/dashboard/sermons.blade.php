            ...
            <table class="table table-striped table-advance table-hover">
              ...
              <thead>
                <tr>
                  ...
                  <th><i class="fa fa-type"></i> Type</th>
              ...
              <tbody id="sermons">
                ...
                @for($i=0; $i < count($data->data); $i++)
                  <tr id="{{$data->data[$i]->id}}">
                    ...
                    <td>
                      @if (strlen($data->data[$i]->topic) < 40) {{$data->data[$i]->topic}} @else {{substr($data->data[$i]->topic, 0, 40)}} ... @endif </td>
                    <td>{{$data->data[$i]->speaker}}</td>
                    <td>{{$data->data[$i]->category->name}}</td>
                    <td>{{date('M', strtotime($data->data[$i]->created_at))}} {{date('d', strtotime($data->data[$i]->created_at)) }}, {{ date('Y', strtotime($data->data[$i]->created_at)) }}</td>
                    <td>
...
<script>
      ...
      success: function(data) {
        ...
        for (let i = 0; i < data.sermons.data.length; i++) {
          $('#sermons').append(`
                  ...
                  <td>${data.sermons.data[i].speaker}</td>
                  <td>${data.sermons.data[i].category.name}</td>
  ...
  $('#sermons').on('click', '.del', function() {
    ...
    let con = confirm("Are you sure you want to delete this sermon?")
    if(!con) return;
    $.ajax({
